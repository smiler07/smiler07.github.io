...
| `CRL-10-REGRESSION` | 전체 Node·syntax·HTML·HTTP·브라우저 회귀 검증 | 15/15 tests, 정적 자원 5/5 HTTP 200 | `DEPLOY_APPROVAL_REQUIRED` |
| `CRL-11-DEPLOY` | 미실행 | commit·push·공개 smoke 미실행 | `DEPLOY_APPROVAL_REQUIRED` |
...
현재 Stop 사유는 오류가 아니라 배포 승인 대기다. commit, push, GitHub Pages 재배포는 실행하지 않았다.
